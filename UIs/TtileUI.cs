...
namespace Waybound.UIs
{
    public class TitleUI : UIState
    {
        ...
        public void DisplayTitle(string text, int displayTime = 1, int fadeTime = 120, float fontScale = 1, int font = 0, Color? altColor = null, Color? altLineColor = null, string subtitle = null)
        {
            if (!Main.dedServ)
            {
                Text = text;
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            ...
            for (int i = centerX - (totalLength / 2) + (totalLength / 2 % 38); i <= centerX + (totalLength / 2) - (totalLength / 2 % 38); i += 38)
            {
                ...
                spriteBatch.Draw(lineTexture, lineDrawPos, lineTexture.Bounds, lineColor * opacity, 0, GetOrigin(lineTexture), FontScale, SpriteEffects.None, 0);
            }
            //draw line endings
            Vector2 leftEndPos = new(centerX - (totalLength / 2) + (totalLength / 2 % 38) - 22, centerY);
            spriteBatch.Draw(leftEndTexture, leftEndPos, leftEndTexture.Bounds, lineColor * opacity, 0, GetOrigin(leftEndTexture), FontScale, SpriteEffects.None, 0);

            Vector2 rightEndPos = new(centerX + (totalLength / 2) - (totalLength / 2 % 38) + 22, centerY);
            spriteBatch.Draw(rightEndTexture, rightEndPos, rightEndTexture.Bounds, lineColor * opacity, 0, GetOrigin(rightEndTexture), FontScale, SpriteEffects.None, 0);
